    ...
    def forward(self, inputs=None, encoder_hidden=None, encoder_outputs=None, function=F.log_softmax,
        ...
        for _ in range(0, max_length):
            ...
            if retain_output_probs:
                stored_outputs.append(log_softmax_output.cpu())
            ...
            if isinstance(hidden, tuple):
                hidden = tuple([h.index_select(1, predecessors.squeeze().cuda()) for h in hidden])
            else:
                hidden = hidden.index_select(1, predecessors.squeeze().cuda())
            ...
            stored_hidden.append(tuple([h.cpu() for h in hidden]))
