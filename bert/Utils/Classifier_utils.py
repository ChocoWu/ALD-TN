    ...
    for (ex_index, example) in enumerate(examples):
        ...
        try:
            if trg_vocab is not None:
                label_id = tokenizer.tokenize(example.text_a)  # 分词
                if len(label_id) > max_seq_length - 2:
                    label_id = label_id[:max_seq_length - 2]
                label_id = ["<sos>"] + label_id + ["<eos>"]
                padding = ['<pad>'] * (max_seq_length - len(label_id))
                label_id += padding
                label_id = [trg_vocab.word_to_id(word) for word in label_id]  # 将词转化为对应词表中的id
            else:
